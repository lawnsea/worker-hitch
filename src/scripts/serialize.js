var serialize = (function () {
    ...
    function getNodeIndex(node) {
        ...
        if (!_.isNumber(index) || parent.__treehouseDirty === true) {
            index = ArrayIndexOf.call(parent.childNodes, node);  
        }
    ...
    function serializeMutationRecord(record, rootNode) {
      var result = {
        type: record.type,
        target: getNodeTraversal(record.target, rootNode),
        addedNodes: _.map(record.addedNodes, serializeNode),
        removedNodes: _.map(record.removedNodes, serializeNode),
      ...
      if (record.previousSibling) {
        result.previousSibling = null;
          getNodeTraversal(record.previousSibling, rootNode);
      }

      if (record.nextSibling) {
        result.nextSibling = null;
          getNodeTraversal(record.nextSibling, rootNode);
      }
    ...
    exports.serializeMutationRecord = serializeMutationRecord;
